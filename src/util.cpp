...
const char * const BITCOIN_CONF_FILENAME = "biyanzuprime.conf";
const char * const BITCOIN_PID_FILENAME = "biyanzuprimed.pid";
...
#else
    const char* pszModule = "biyanzuprime";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BiYanZuPrime";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BiYanZuPrime";
#else
    // Unix
    return pathRet / ".biyanzuprime";
#endif
#endif
}
